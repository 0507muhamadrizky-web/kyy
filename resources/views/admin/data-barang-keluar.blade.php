        ...
        <div class="py-12">
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <div class="custom-table-container">
                                <h2 class="text-lg font-bold text-green-800 mb-6">Data Barang Keluar</h2>
                                ...
                                <table class="custom-table" id="barangKeluarTable">
                    <thead>
                        <tr>
                            <th>User</th>
                    ...
                    <tbody>
                        @foreach(\App\Models\BarangKeluar::with('user')->latest()->get() as $bk)
                        <tr>
                            <td>{{ $bk->user->name ?? '-' }}</td>
                            <td>{{ $bk->nama_penginput }}</td>
                            <td>{{ $bk->nama_barang }}</td>
                            <td>{{ $bk->jumlah_barang }}</td>
                            <td>
                                @if($bk->foto_bukti)
                                    <a href="{{ asset('storage/'.$bk->foto_bukti) }}" target="_blank" class="text-blue-600 underline"><i class="bi bi-eye"></i> Lihat Foto</a>
                                @else
